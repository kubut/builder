<?php
namespace BuilderBundle\AsyncAction;
...
/**
 * Class Async
 * @package BuilderBundle\AsyncAction
 */
class Async
{
    ...
    private $_errors = array(
        Async::FUNCTION_NOT_CALLABLE => 'You must specify a valid function name that can be called from the current scope.',
        Async::COULD_NOT_FORK        => 'pcntl_fork() returned a status of -1. No new process was created'
    );
    ...
    {
        if(!Async::isAvailable() )throw new Exception("Asyncs not supported");
        if ( $runnable !== null ) {
    ...
    public function setRunnable( $runnable )
    {
        if ( self::isRunnableOk($runnable) ) {
            ...
            $this->fatalError(Async::FUNCTION_NOT_CALLABLE);
        }
    }
    ...
    public function start()
    {
        $pid = @pcntl_fork();
        if ( $pid == -1 ) {
            $this->fatalError(Async::COULD_NOT_FORK);
        }
        if ( $pid ) {
            // parent
            $this->_pid = $pid;
